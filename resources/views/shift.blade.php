...
<body class="bg-gray-50 font-sans">
            ...
            <div class="flex justify-between items-center bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('schedule') }}"
                        class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition font-bold flex items-center gap-2">
                        <i class="far fa-calendar-alt"></i> Kalender
                    </a>
                    <button onclick="openAddModal()"
                        class="px-5 py-2.5 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition font-bold flex items-center gap-2">
                            ...
                            @foreach($shifts as $shift)
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="p-4 text-center">
                                        <div class="w-6 h-6 rounded-full mx-auto shadow-sm"
                                            style="background-color: {{ $shift->color }};" title="{{ $shift->color }}">
                                        </div>
                                    </td>
                                    <td class="p-4">
                                    ...
                                    <td class="p-4 text-center">
                                        ...
                                        @if($shift->is_cross_day)
                                            <span
                                                class="text-[10px] text-purple-600 bg-purple-100 px-2 py-0.5 rounded-full font-bold border border-purple-200">
                                                +1 Hari (Lintas Hari)
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="p-4 text-center">
                                        <span
                                            class="bg-cyan-100 text-cyan-800 text-xs px-3 py-1 rounded-full font-bold border border-cyan-200 uppercase"> {{ $shift->branch->name }}
                                        </span>
                                    </td>
                                    <td class="p-4">
                                        <div class="flex justify-center items-center gap-2">
                                            ...
                                            <form action="{{ route('delshift', $shift->id) }}" method="POST"
                                                ...
                                                <button type="button"
                                                    ...
                                                    <i class="fas fa-trash text-lg"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
    ...
    <div id="shiftModal"
        ...
        <div class="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl relative transform transition-all scale-100">
            <button onclick="document.getElementById('shiftModal').classList.add('hidden')"
            ...
            <form id="shiftForm" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name"
                        class="w-full rounded-lg border-gray-300 p-2.5 border focus:ring-2 focus:ring-blue-500"
                ...
                <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 space-y-3">
                    ...
                    <div class="flex gap-2">
                        <input type="color" name="color" id="color"
                            class="h-10 w-12 rounded cursor-pointer border border-gray-300 p-0" value="#3B82F6">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer mt-2 border-t border-gray-200 pt-2">
                        <input type="checkbox" name="is_cross_day" id="is_cross_day" value="1"
                            class="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
                        <span class="text-sm text-gray-700 font-medium">Lintas hari (Berakhir besok)</span>
                    </label>

                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1 mt-2">Cabang yang Berlaku
                            (Opsional)</label>
                        <select name="branch_id" id="branch_id"
                            class="w-full rounded-lg border-gray-300 p-2 border focus:ring-2 focus:ring-blue-500 text-sm"
                            required>
                ...
                <div class="pt-2">
                    <button type="submit"
                        class="w-full py-2.5 bg-blue-600 text-white font-bold rounded-lg shadow hover:bg-blue-700 transition">Save
                        Shift</button>
                </div>
            </form>
        </div>
